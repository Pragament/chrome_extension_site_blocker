async function run() {
  ...
  const endpoint = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/students?pageSize=20`;
  ...
  if (!res.ok) {
    console.error("Fetch students failed:", res.status, await res.text());
    return;
  }
